...
class PreProcess:
    ...
    def pipeline(self, path, idx0):
        ...
        try:
            ...
            println("[ล้มเหลว/Error] เกิดข้อผิดพลาดกับไฟล์ %s (ระบบจะทำการลบไฟล์นี้ทิ้งอัตโนมัติ):\n%s" % (path, traceback.format_exc()), self.log_path)
            try:
                os.remove(path)
                println("[ลบไฟล์สำเร็จ] ลบไฟล์แปลกปลอม/ไฟล์เสียเรียบร้อยแล้ว: %s" % path, self.log_path)
            except Exception as ex:
                println("[ล้มเหลว/Error] ไม่สามารถลบไฟล์ %s ได้: %s" % (path, ex), self.log_path)
